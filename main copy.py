...
Two-part identity (see extract_points):
  ...
  derived_hints.module_name is inferred from the page name (e.g. 16DI-1-UP ->
    ...
    Instance.eod), as is proving active/deleted page status.
...
def build_page_map(page_eod_path):
    """Return (page_map, stats) where page_map = {page_object_id: page_name}."""
    ...
    for i in range(slots):
        ...
        page_map[pid] = m.group(1).decode("latin1", "replace") if m else None
...
def extract_points(eod_path, page_map=None, only_ddc_nums=None):
    ...
    page_map: {page_object_id: page_name} from build_page_map (optional).
...
# A drawing enumerates each physical device of a grouped schedule item as its
# own wiring point, tagged with an instance number: the equipment type becomes
# singular + "-N" (e.g. "Submersible Pump-3", "Fire Fighting Panel-2") or the
# point tag gains a "-N" suffix (e.g. "Pump On/Off Status-2"). The schedule lists
# only the grouped item once (e.g. "Submersible Pumps 2 Sets (1 Duty & 1
# Standby)"). These per-instance records carry NO structural flag distinguishing
# them from the grouped point - they differ only by this naming - so we exclude
# them by the instance-number suffix.
_INSTANCE_SUFFIX = re.compile(r"-\s*\d+$")


def keep_schedule_points(points):
    """
    Reduce raw EPLAN records to schedule-level points by dropping ONLY the
    per-instance wiring records (instance-number suffix on the equipment type or
    point tag). Exact duplicates are deliberately KEPT: a point that the drawing
    actually contains twice is either a real duplication error or two distinct
    points sharing a name - both must be surfaced (see verify's duplicates
    report), never silently removed. Returns (kept, dropped).
    """
    kept, dropped = [], []
    for p in points:
        sch = p["schedule_identity"]
        etype = sch.get("equipment_type") or ""
        ptag = sch.get("point_tag") or ""
        if _INSTANCE_SUFFIX.search(etype) or _INSTANCE_SUFFIX.search(ptag):
            dropped.append({**p, "excluded_reason": "instance-number wiring point"})
            continue
        kept.append(p)
    return kept, dropped
...
# NO UserSupplementaryFields (TOK_SUPP_FIELDS), so keep_schedule_points/
# extract_points correctly never see it as a point. Its own designation number
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print("[3/7] Extracting raw point records from EPLAN ...")
        raw_points = extract_points(func_eod, page_map=page_map,
                                    only_ddc_nums=ddc_nums or None)
        target_terminals = extract_terminal_designations(
            func_eod, page_map=page_map, only_ddc_nums=ddc_nums or None)
    print(f"      {len(raw_points)} raw records on the scoped DDC page(s)")
    print(f"      {len(target_terminals)} terminal designations on the scoped DDC page(s)")

    print("[4/7] Reducing to schedule-level points "
          "(drop per-instance wiring records) ...")
    points, excluded = keep_schedule_points(raw_points)
    ...
    print(f"      kept {len(points)} points -> {OUT_JSON}")
    print(f"      excluded {len(excluded)} per-instance wiring point(s) -> {OUT_EXCLUDED}")
    ...
    if REFERENCE_SOURCE is None:
        ...
        with tempfile.TemporaryDirectory() as tmp2:
            ...
            reference_terminals = extract_terminal_designations(
                ref_func_eod, page_map=ref_page_map, only_ddc_nums=ddc_nums or None)
        terminal_result = terminal_diagnostics(target_terminals, reference_terminals)
        terminal_has_diff = terminal_result["has_differences"]
        report["terminal_diagnostics"] = {
            "note": "REFERENCE-based diff of terminal ('TR') designations against "
                    "REFERENCE_SOURCE. PRIMARY key is (page_object_id, "
                    "function_object_id) -> terminal_number (catches swaps); the "
                    "page multiset is a rollup/fallback. Extraction is certified.",
            "reference_source": REFERENCE_SOURCE,
            "reference_terminal_count": len(reference_terminals),
            "target_terminal_count": len(target_terminals),
            **terminal_result,
        }
        print(f"      reference terminals: {len(reference_terminals)}, "
              f"target terminals: {len(target_terminals)}")
    ...
    # nothing missing/extra, every page reference resolved and consistent, no
    # ambiguous legacy page-name variants, the schedule itself is internally
    # consistent (no INVALID_EXCEL_TYPE rows, no QTY*flag/Total mismatches), and
    # - when a reference was supplied - no terminal-numbering differences.
    ...
    print(f"      EPLAN points (scoped) : {report['eplan_points']}")
